...
<iframe src="https://jandimter.shinyapps.io/Cohesion3/" style="border: simple; width: 1135px; height: 850px"></iframe>
...
dta_2023<- list.files(path = "data/LAPOP2023/", pattern = ".dta")
dta_2023 <- paste0(file = "data/LAPOP2023/", dta_2023)
...
vars_mod <- names(prom_wave)[c(3:22,23:29)]
...
# Se almacena la data.
write_rds(prom_wave,"data/Lapop_data_proms_2023.RDS")
```

```{r Creación descriptivos LAPOP, results='asis'}
LAPOP <- read_rds("data/Lapop_data_proms_2023.RDS") # Promedios por ola-país
```
...
LAPOP <- LAPOP[order(LAPOP$pais),] %>%
        ...
        select(-c(b43,vicbar1,vicbar3,vicbar7,jamb18a,b23,b39n,nicfear, movus1))
```
...
los datos del 2021, se excluyó esta ola del análisis**
...
tabla_prop_perdidos <- data.frame(
  Variable = colnames(LAPOP),
  Proporcion_Casos_Perdidos = sapply(LAPOP, prop_casos_perdidos)
)%>%t()
```

```{r Almacenado datos LAPOP}
saveRDS(LAPOP,"data/data_lapop_2023.rds")
```
...
redistribucion <- data.frame(     
  variable = c("p17st","p26st.e","p14st","P12ST","P12ST","P27ST","P18ST", 
               "P21ST","P20ST","P23ST", "P17ST"),     
  año = c(2007, 2008, 2009, 2010, 2011, 2013, 2015, 2016, 2017, 2018, 2023),     
  descripcion = c(
    ...
    "Cómo de justa es la distribución del ingreso en el país",# 1 Muy justa 4 Muy injusta                   
    "Cómo de justa es la distribución del ingreso en el país",# 1 Muy justa 4 Muy injusta                   
    "Cuán justa es la distribución del ingreso en {PAÍS}" # 1 Muy justa 4 Muy injusta
  ),     
  standard = 'Es justa la distribucion de ingreso/riqueza',     
  code = 'justiciaingresoriqueza'
)  # Comentario movido fuera del paréntesis


# Frecuencia de hablar sobre política
hablarpolitica <- data.frame(
  variable = c("p71sta","p37st.aa","p61st","P30TGB.A","P20TGB.A", "P44ST.A"),
  año = c(2005,2006,2007,2013,2015, 2023),
  descripcion=c(rep('Frecuencia con que habla de política',3),
                rep("Frecuencia con la que hace las siguientes cosas: hablar de
                 ...
                 "Frecuencia con que habla de política con los amigos"),
...
convencerpoliticamenre <-
  data.frame(
    variable = c("p71stb", "p37st.ab", "P30ST.C", "P20ST.C", "P44ST.C"),
    año = c(2005, 2006, 2013, 2015,2023),
    descripcion = c(
      ...
      "recuencia con que trata de convencer políticamente"),
...
firmarpeticion <-
  data.frame(
    variable = c("p72sta","p37st.ba","p62st.a","p82wvsta","P32STGBS","P21TGB.A", "P45ST.A"),
    año = c(2005, 2006, 2007, 2008, 2013, 2015,2023),
    descripcion = c(
...
apoyodemocracia <- data.frame(
  variable = c("p13st", "p16st", "p17st", "p9st", "p13st", "p10st", "P10ST",
             "P11STGBS", "P12STGBS", "P12STGBS", "P8STGBS", "P8STGBS", "P10STGBS"),
  año = c(2004,2005,2006,2007,2008,2009,2010,2015,2013,2018,2016,2017,2023),
  descripcion = c(rep("Apoyo a la democracia")),
...
prom_wave_latin <- prom_wave_latin %>% filter(wave!=2020)

saveRDS(prom_wave_latin,"data/data_latinobarometro_2023.rds")
```
...
data <- WVS %>% select(COW_NUM,S020,S017,E035,G006,H001, E235,
                       E025, E026, E027, E028, E040, A165,
                       F114A, F115, F116, F117)
...
data <- data %>%
  setNames(c("pais","wave","wt","igualdadingresos","orgullonacional","seguridadbarrio", "importanciademocracia", "firmapeticion_wvs", "boycotts",
             "protestaspacificas", "huelgas", "trabajoduro",
             "confianza_generalizada",
             "recibir_beneficios", "evadir_transporte", "evadir_impuestos", "aceptar_soborno"))
...
Se aplica correlación de Spearman a indicadores que constituyen una subdimensión. Los valores son representativos para todos los países y todos los años estudiados.
...
chart.Correlation(datos %>% select(aoj11,sd2new2,sd3new2,sd6new2) %>%
                    setNames(c("Perception of Neighborhood Insecurity", "Satisfaction with Roads",
                             "Satisfaction with Public Schools","Satisfaction with Public Medical and Health Services")))
```
...
chart.Correlation(datos %>% select(pn4, b4) %>%
...
calidad_year_n <- datos %>%
  ...
  summarise(sub_orgullonacional = sum(!is.na(c(b4,orgullonacional))),                                            #2
            sub_satisfaccionresidencial = sum(!is.na(c(aoj11, sd2new2, sd3new2,sd6new2))),       #5
            sub_confianzainterpersonal = sum(!is.na(c(it1))),                                                    #1
            sub_comportamientoprosocial = sum(!is.na(c(recibir_beneficios,
                                                     evadir_impuestos,
                                                     evadir_transporte,
                                                     aceptar_soborno))),                                                   #1
            ...
            sub_participacioncivica = sum(!is.na(c(protestaspacificas,
                                                   huelgas,
                                                   firmapeticion_wvs))), #4
            sub_interespolitico = sum(!is.na(c(pol1, hablarpolitica))),                                                     #2
            sub_justiciadistributiva = sum(!is.na(c(justiciaingresoriqueza,igualdadingresos))),                  #2
            sub_surgirenlavida = sum(!is.na(c(trabajoduro))),                                            #2
            sub_justiciaprocedimental= sum(!is.na(c(exc7)))
            )
```
...
table <- calidad_year_n %>%
                  ...
                  setNames(c("País","Ola",
                   ...
                   "Justicia distributiva","Surgir en la vida", "Justicia procedimental")) 
```
...
for(i in unique(prom$pais)){
  ...
  part <- data.frame(
    ...
    sub_satisfaccionresidencial = round(sum(c(part$aoj11, part$sd2new2, part$sd3new2, part$sd6new2)) / (4 * n_years) * 100, 1),
    sub_confianzainterpersonal = round(sum(part$it1) / (1 * n_years) * 100, 1),
    sub_comportamientoprosocial = round(sum(c(part$recibir_beneficios, part$evadir_transporte, part$evadir_impuestos, part$aceptar_soborno)) / (4 * n_years) * 100, 1),
    sub_confianzainstitutciones = round(sum(c(part$b21a, part$b21, part$b18, part$b10a, part$b13)) / (5 * n_years) * 100, 1),
    sub_actitudhacialademocracia = round(sum(c(part$pn4, part$b4, part$apoyodemocracia)) / (3 * n_years) * 100, 1),
    sub_participacioncivica = round(sum(c(part$firmapeticion_wvs, part$protestaspacificas, part$huelgas)) / (3 * n_years) * 100, 1),
    sub_interespolitico = round(sum(c(part$pol1, part$hablarpolitica)) / (2 * n_years) * 100, 1),
    sub_justiciadistributiva = round(sum(c(part$justiciaingresoriqueza, part$igualdadingresos)) / (2 * n_years) * 100, 1),
    sub_surgirenlavida = round(sum(part$trabajoduro) / (1 * n_years) * 100, 1),
    sub_justiciaprocedimental = round(sum(part$exc7) / (1 * n_years) * 100, 1)
  )
...
calidad_year <- prom %>%
  ...
  mutate(
    ...
    sub_satisfaccionresidencial  = ifelse(rowSums(cbind(aoj11,sd2new2,sd3new2,sd6new2)) > 2, FALSE, TRUE),
    sub_confianzainterpersonal   = ifelse(rowSums(cbind(it1)) > 0, FALSE, TRUE),
    sub_comportamientoprosocial  = ifelse(rowSums(cbind(evadir_impuestos,
                                                        evadir_transporte,
                                                        aceptar_soborno,
                                                        recibir_beneficios)) > 1, FALSE, TRUE),
    ...
    sub_participacioncivica      = ifelse(rowSums(cbind(firmapeticion_wvs,
                                                        protestaspacificas,
                                                        huelgas)) > 1, FALSE, TRUE),
    sub_interespolitico          = ifelse(rowSums(cbind(pol1, hablarpolitica)) > 1, FALSE, TRUE),
    sub_justiciadistributiva     = ifelse(rowSums(cbind(justiciaingresoriqueza, igualdadingresos)) > 1, FALSE, TRUE),
    sub_surgirenlavida           = ifelse(rowSums(cbind(trabajoduro)) > 0, FALSE, TRUE),
    sub_justiciaprocedimental    = ifelse(rowSums(cbind(exc7)) > 0, FALSE, TRUE)
  ) %>%
...
for(i in 1:nrow(prom)){
  # print(i)
  for(x in names(calidad_year)[3:13]){
    if(calidad_year[i,x]){
      prom[i,x] <- NA
    }
  }
...
save(prom, file="data/base_subdimensiones_rev.RData")
```
...
calidad_year_dims <- prom %>%
  ...
  mutate(
    ...
    dim_percepcionjusticia                = ifelse(rowSums(cbind(sub_justiciadistributiva, sub_surgirenlavida,
                                                                 sub_justiciaprocedimental)) > 1, FALSE, TRUE)
  ) %>%
...
save(prom, file="data/base_dimensiones_rev.RData")
```
...
save(prom, file="data/base_cohesion_rev.RData")
```
...
save(prom, file= "data/indicadores_cohesion_rev.RData")
```


```{r}
  calidad[1:12] %>%
  setNames(c("País","Orgullo nacional","Satisfacción residencial","Confianza interpersonal","Comportamiento prosocial","Confianza en instituciones","Actitud hacia la democracia","Participación cívica","Interés político","Justicia distributiva","Surgir en la vida", "Justicia Procedimental")) %>%
  flextable() %>%
  theme_alafoli() %>%
  autofit() %>% 
  align(align = "center", part = "all") %>%
  ...
  bg(i = ~ `Justicia Procedimental` < 75, j = 11, bg = "#F4D03F") %>%
  color(i = ~ `Justicia Procedimental` < 75, j = 11, color='#444444')%>%
  ...
  color(i = ~ `Surgir en la vida` < 50, j = 11, color='white') %>%
   bg(i = ~ `Justicia Procedimental` < 50, j = 11, bg = "#E73C6D") %>%
  color(i = ~ `Justicia Procedimental` < 50, j = 11, color='white')
...
saveRDS(prom, "data/indicadores_cohesion.RDS")
```
