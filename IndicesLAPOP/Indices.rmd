...
[Latinobarómetro](https://www.latinobarometro.org/lat.jsp) y de la [Encuesta Mundial de Valores (WVS)](https://www.worldvaluessurvey.org/wvs.jsp). A
...
este estudio. Además, tomar como referencia sus olas (cada
dos años) para la periodización de la información.
...
objetivo de este trabajo. A partir de esta revisión se
determinará si son aptas para su utilización.
...
Se incorpora en tabla países presentes en LAPOP como
referencia.
...
años siguiendo el esquema LAPOP. Se hace una excepción con Uruguay
...
los valores máximos. Ej. Para `Orgullo nacional` el 50,5% de
los casos (país-ola) tienen un solo indicador para la
construcción de la subdimensión. El 49,5% restante de los
casos tiene el la totalidad de indicadores necesarios (2).
...
A partir del análisis sobre disponibilidad, para la
estimación de subdimensiones se define un estándar del 60%
de los datos disponibles para considerarse válido. Las
subdimensiones resultantes (país-ola) con una proporción
menor son recodificados como casos perdidos (NAs). Los resultados son presentados en el dashboard inserto al [inicio del documento](#title-block-header)
