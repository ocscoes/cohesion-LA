...
```{r Disponibilidad de indicadores LAPOP por año}
...
plot <- ggplot(review_summary,
        ...
        ggtitle(paste0(
          ...
          as.character(get_label(merge)[i]))) +
        labs(
          x = "Ola",
...
**Debido a la baja presencia de las variables de interés en los datos del 2021se trabajará tomando como límite la ola 2018.**

Habiendo definido los indicadores LAPOP a usar, se revisa ahora su disponibilidad por país:

```{r Disponibilidad datos por país LAPOP, message=FALSE}
...
# Reminder: prom_wave es el objeto base que se carga de los datos elaborados LAPOP.
...
saveRDS(prom,"data/data_lapop.rds")
```
...
rm(list=setdiff(ls(),c("prom","prop_casos_perdidos","LAPOP_escalaoriginal",
               "standardize_data","impute_data","mean_na")))
```


```{r Variables Latinobarómetro a utilizar, warning=FALSE, message=FALSE, include = FALSE}
...
saveRDS(data_latinobarometro,"data/data_latinobarometro.rds")
```
...
Para la estimación de subdimensiones se define un estándar del 60% de los datos disponibles para considerarse válido. Los casos (a nivel país-año) con una proporción menor son recodificados como casos perdidos (NAs). A continuación se presenta la visualización del resultado:
...
```{r test bivariado, eval=FALSE}
# Inicialización del gráfico
p <- plot_ly()

# Valores predeterminados para la inicialización
x_default <- unique(df$Variable)[3]
y_default <- unique(df$Variable)[3]
year_default <- min(df$Ola)

# Añadir trazas basadas en valores predeterminados
# Añadir trazas basadas en valores predeterminados
for(pais in unique(df$País)){
  temp_df <- df %>% filter(País == pais, Ola == year_default)
  p <- add_trace(
    p,
    x = temp_df[[x_default]],
    y = temp_df[[y_default]],
    name = pais,
    type = 'scatter',
    mode = 'markers',   # Cambio aquí: de 'lines' a 'markers'
    marker = list(color = sample(colors(), 1)),
    hovertemplate = paste("País: ", pais,
                          "<br>Año: ", year_default, 
                          "<br>", x_default, ": %{x}",
                          "<br>", y_default, ": %{y}<extra></extra>")
  )
}


# Crear menús desplegables
# Menú desplegable para el eje X
buttons_x <- lapply(unique(df$Variable), function(variable) {
  y_values <- unlist(lapply(unique(df$País), function(pais) {
    df %>% filter(País == pais, Ola == year_default) %>% `[[`(y_default)
  }))
  
  list(
    method = 'restyle',
    args = list('x', df %>% filter(Ola == year_default) %>% `[[`(variable),
                'y', y_values),
    label = variable
  )
})

# Menú desplegable para el eje Y
buttons_y <- lapply(unique(df$Variable), function(variable) {
  x_values <- unlist(lapply(unique(df$País), function(pais) {
    df %>% filter(País == pais, Ola == year_default) %>% `[[`(x_default)
  }))
  
  list(
    method = 'restyle',
    args = list('x', x_values,
                'y', df %>% filter(Ola == year_default) %>% `[[`(variable)),
    label = variable
  )
})
...
# 3. Menú desplegable para el año
buttons_year <- lapply(unique(df$Ola), function(year) {
  list(
    method = 'restyle',
    args = list('x', df %>% filter(Ola == year) %>% `[[`(x_default),
                'y', df %>% filter(Ola == year) %>% `[[`(y_default)),
