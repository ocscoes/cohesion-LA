...
A partir del análisis sobre disponibilidad, para la estimación de subdimensiones se define un estándar del 60% de los datos disponibles para considerarse válido. Las subdimensiones resultantes (país-ola) con una proporción menor son recodificados como casos perdidos (NAs). A continuación se presenta la visualización del resultado:
...
```{r Shiny mapa}
# Asegúrate de que 'subdim_values' esté definido de acuerdo con tus datos
subdim_values$iso3 <- countrycode(subdim_values$País, 'country.name', 'iso3c')
world_data <- ne_countries(scale = "medium", returnclass = "sf")
world_data <- st_transform(world_data, crs = 4326)
names(world_data)[names(world_data) == "iso_a3"] <- "iso3"
world_data <- world_data %>% select(iso3, geometry)
# Asegúrate de que esta fusión funcione según tu estructura de datos
world_data <- merge(world_data, subdim_values, by = "iso3")
...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(6, style = "text-align: center;",
           actionButton("btn_var1", label = HTML('<img src="Orgullo nacional.svg" alt="Orgullo nacional" width="30">'), value = "Orgullo nacional"),
           actionButton("btn_var2", label = HTML('<img src="Satisfacción residencial.svg" alt="Satisfacción residencial" width="30">'), value = "Satisfacción residencial")
    )
  ),
